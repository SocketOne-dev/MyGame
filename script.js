 ...
 * @param {string|null} reason - Причина завершения игры (например, 'quit' если нажат крестик)
 */
function endGame(reason = null) {
  ...
  if (reason === 'quit') {
    // Игрок нажал на крестик (вышел сам)
    resultTitle.textContent = "Игра прервана 🛑";
    resultMessage.textContent = "Вы вышли из игры до её завершения. Но ваш текущий счет сохранен!";
  } else if (lives === 0) {
...
btnQuit.onclick = () => endGame('quit'); // Досрочное завершение игры с передачей причины 'quit'
